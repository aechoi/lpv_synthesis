...
def LFT(Grho_tilde_modified, controller):
    ...
    A_LFT = sp.BlockMatrix(
        [
            [A + B2 @ invprod @ Dk @ C2, B2 @ invprod @ Ck],
            [Bk @ C2 + Bk @ D22 @ invprod @ Dk @ C2, Ak + Bk @ D22 @ invprod @ Ck],
        ]
    )
    B_LFT = sp.BlockMatrix(
        [[B1 + B2 @ invprod @ Dk @ D21], [Bk @ D21 + Bk @ D22 @ invprod @ Dk @ D21]]
    )
...
if __name__ == "__main__":
    ...
    controller = controller_sample(
        sp.Matrix([0]), sp.Matrix([0]), sp.Matrix([0]), sp.Matrix([3])
    )
